...
Chef::Log::info("Donwloading #{node['vagrant_magento']['source']['url']} to #{Chef::Config[:file_cache_path]}/magento-source.tar.bz2 ...")
remote_file "#{Chef::Config[:file_cache_path]}/magento-source.tar.bz2" do
  source "#{node['vagrant_magento']['source']['url']}"
  action :create_if_missing
  backup false
  mode "0644"
  notifies :run, 'execute[magento-extract]'
...
execute "magento-extract" do
  Chef::Log::info("Extracting Magento to #{node['vagrant_magento']['mage']['dir']} ... ")

  # TODO: Fix directory creation - not sure why this throws an error. Sorin?!
  #directory "#{node['vagrant_magento']['mage']['dir']}" do
  #  mode 00644
  #  action :create
  #end
  command "tar xvf #{Chef::Config[:file_cache_path]}/magento-source.zip -C #{node['vagrant_magento']['mage']['dir']}"

  not_if { node['vagrant_magento']['source']['install'] == false }
  only_if { File.exits?("#{Chef::Config[:file_cache_path]}/magento-source.zip") }

  action :nothing
  notifies :run, 'ruby_block[sample-data]'
end
...
execute "magento-install" do
  ...
  Chef::Log::Info("Install magento #{node['vagrant_magento']['config']['install']}")
